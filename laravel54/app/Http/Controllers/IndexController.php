...
class IndexController extends Controller
{
    ...
    {
        // Cache::store('memcache')->put('no','klsdfj',2);
        // echo Cache::store('memcache')->get('name');
        // echo 3;die;
        
        
        $res = DB::select("select * from huya_anchor");
//      $res = DB::table('anchor')->first();
//      print_r($res);die;
        $user = array();
...
//      print_r($data);die;
    ...
    public function category(){
        ...
        // echo sizeof('字');die;
        // echo strlen('字');
        // $res = str_repeat('字',100000);
        // echo strlen($res),'kb';die;
        /*293M key*/
        // echo ceil(strlen($res)/1024),'M';die;
        // Cache::store('memcache')->put($res,'3000kb11',1);

        // echo Cache::store('memcache')->get($res);die;
        // var_dump(Cache::store('memcache')->get('cate'));die;
        /*如果缓存中存在*/
        $data = Cache::store('memcache')->get('cate');
        if( !$data ) {
            $rs=DB::select("select * from huya_type where type_status=:status",['status'=>1]);
            $data=$this->cate($rs);
            Cache::store('memcache')->put('cate',$data,1440);
        }
        /* else {
            $data = $mem;
            // echo ;
